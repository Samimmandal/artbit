...
export default function HomePage() {
  ...
  const loadData = async () => {
    try {
      const [prodsRes, artRes] = await Promise.all([
        supabase
          .from('products')
          .select('*')
      ...
      if (artRes.error) {
        console.error('homepage_art error:', artRes.error)
      }
  ...
  return (
    <div className={`min-h-screen ${bg} ${text}`}>
      <header className={`border-b ${border} sticky top-0 ${bg} z-50`}>
        <div className="max-w-6xl mx-auto px-5 sm:px-6 py-3.5 flex items-center justify-between gap-3">
          ...
          <nav className={`hidden md:flex items-center gap-6 text-xs font-mono uppercase tracking-wider ${muted}`}>
            ...
            <Link href="/shop" className="hover:opacity-100 opacity-80 transition">Custom Prints</Link>
      ...
      {/* Hero — BRING YOUR OWN ART */}
      <section className="max-w-6xl mx-auto px-5 sm:px-6 py-12 md:py-20">
        <div className="grid md:grid-cols-2 gap-10 md:gap-14 items-center">
          <div>
            <p className="text-xs font-mono uppercase tracking-widest mb-4 text-[#e2a233]">
              Bring your own art
            </p>
            ...
            <Link
              href="/shop"
              className="inline-block bg-[#e2a233] text-[#000000] px-6 py-3.5 font-mono text-xs uppercase tracking-wider font-semibold"
            >
              Start your design
            </Link>
          </div>
          ...
          {/* Dynamic Your Art */}
          <div className="aspect-square w-full max-w-md mx-auto overflow-hidden relative border border-white/10">
            {homeArt?.image_url ? (
              <img
      ...
      <section id="process" className={`border-t ${border}`}>
        <div className="max-w-6xl mx-auto px-5 sm:px-6 py-14 md:py-16">
          <p className={`text-xs font-mono uppercase tracking-widest mb-2 ${muted}`}>How it works</p>
          <h2 className="text-2xl sm:text-3xl font-black uppercase mb-10">The Artbit process</h2>
      ...
      <section className={`border-t ${border}`}>
        <div className="max-w-6xl mx-auto px-5 sm:px-6 py-14 text-center">
          <h2 className="text-2xl sm:text-3xl font-black uppercase mb-4">Ready to wear your art?</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-10">
            ...
            <div>
              ...
              <ul className={`space-y-2 text-sm ${muted}`}>
                <li><Link href="/shop">Tees</Link></li>
                <li><Link href="/shop">Hoodies</Link></li>
            ...
            <div>
              ...
              <ul className={`space-y-2 text-sm ${muted}`}>
                ...
                <li><Link href="/page/privacy">Privacy</Link></li>
                <li><Link href="/page/hiring">We&apos;re Hiring</Link></li>
              </ul>
            </div>
            <div>
              ...
              <div className="flex gap-3">
                <a href="https://www.instagram.com/artbit.co.in?igsh=ZHJyNXFhb2VwY2xr" target="_blank" rel="noopener noreferrer" className={iconCls} aria-label="Instagram">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="1" fill="currentColor" stroke="none"/></svg>
                </a>
                <a href="https://www.facebook.com/share/19Eop63Sz3/" target="_blank" rel="noopener noreferrer" className={iconCls} aria-label="Facebook">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"/></svg>
                </a>
                <a href="mailto:artbit.hq@gmail.com" className={iconCls} aria-label="Email">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
                </a>
              </div>
            </div>
          </div>
